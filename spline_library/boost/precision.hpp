   ...
   namespace math
   {
      namespace tools
      {
         ...
         namespace detail
         {
            ...
            template <class T>
            struct log_limit_noexcept_traits_imp<T, false> : public std::integral_constant<bool, false>
            {
            };
...
#else
            return ::std::numeric_limits<T>::is_specialized ? detail::epsilon<T>(std::true_type()) : detail::epsilon<T>(std::false_type());
#endif
         }
